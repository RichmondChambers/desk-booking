...
from utils.firestore_users import list_users, migrate_sqlite_users, update_user
...
for user in users:
    is_self = user.email == current_user_email

    with st.container(border=True):
        col1, col2, col3, col4, col5, col6 = st.columns([3, 4, 2, 2, 2, 3])

        col1.markdown(f"**{user.name}**")
        col2.markdown(user.email)
        col3.markdown(f"Role: **{user.role}**")
        col4.markdown(f"Can book: **{'Yes' if user.can_book else 'No'}**")
        col5.markdown(f"Active: **{'Yes' if user.is_active else 'No'}**")
        ...
        with col6:
            ...
            if user.role == "admin":
                if st.button(
                    "Remove admin",
                    key=f"remove_admin_{user.user_id}",
                    disabled=is_self,
                ):
                    if not is_self:
                        log_action("REMOVE_ADMIN", f"Removed admin role from {user.email}")
                        update_user(user.email, {"role": "user"})
                        st.rerun()
            else:
                if st.button(
                    "Make admin",
                    key=f"make_admin_{user.user_id}",
                    disabled=is_self,
                ):
                    log_action("PROMOTE_TO_ADMIN", f"Promoted {user.email} to admin")
                    update_user(user.email, {"role": "admin"})
                    st.rerun()

            # BOOKING PERMISSION
            toggle_label = "Disable booking" if user.can_book else "Enable booking"
            if st.button(
                toggle_label,
                key=f"toggle_booking_{user.user_id}",
                disabled=is_self or not user.is_active,
            ):
                if not is_self:
                    log_action(
                        "TOGGLE_CAN_BOOK",
                        f"{'Disabled' if user.can_book else 'Enabled'} booking for {user.email}",
                    )
                    update_user(user.email, {"can_book": not user.can_book})
                    st.rerun()

            # ACTIVE STATUS
            status_label = "Deactivate user" if user.is_active else "Activate user"
            if st.button(
                status_label,
                ...
                if not is_self:
                    log_action(
                        "TOGGLE_ACTIVE",
                        f"{'Deactivated' if user.is_active else 'Activated'} user {user.email}",
                    )
                    if user.is_active:
                        update_user(user.email, {"is_active": False, "can_book": False})
                    else:
                        update_user(user.email, {"is_active": True})
                    st.rerun()
...
with st.expander("Migrate existing SQLite users to Firestore"):
    st.caption(
        "Run once to copy existing users from the local SQLite database. "
        "This does not delete local data."
    )
    if st.button("Run user migration"):
        conn = get_conn()
        rows = conn.execute(
            """
        ...
        stats = migrate_sqlite_users(rows)
        st.success(
            f"Migrated users. Inserted {stats['inserted']}, "
            f"skipped {stats['skipped']} existing records."
        )
